...
function App() {
  ...
  useEffect(() => {
    // Vérification retour paiement PayTech uniquement
  ...
  // 1. RECHERCHE DYNAMIQUE (CACHE LES RÉSULTATS SI VIDE)
  const chercherPartition = async (texte) => {
    setSearch(texte);
    
    // CONDITION : Si le champ est vide, on vide la liste des résultats
    if (texte.trim() === "") {
      setResultats([]); 
      return;
    }
    ...
    const { data } = await supabase
      ...
      .limit(20); // Limite pour la performance
  ...
  const handleActionPaiement = async () => {
    ...
    if (adminInput === SECRET_ADMIN_CODE) { setIsPaid(true); return; }
    setUploading(true);
    const urlWave = await genererLienPaiement(newTitre);
    if (urlWave) window.location.href = urlWave;
    else { alert("Erreur PayTech."); setUploading(false); }
  };
  ...
  const finaliserPublication = async (e) => {
    ...
    try {
      ...
      alert("✅ Publié !");
      setIsPaid(false); setShowAdmin(false); setNewTitre(""); setNewAuteur(""); setFile(null);
  ...
  return (
    <div style={styles.container}>
      ...
      {showAdmin && (
        <div style={styles.adminPanel}>
          <div style={styles.modalHeader}>
            <h3 style={{margin:0}}>{isPaid ? "Étape 2" : "Étape 1"}</h3>
            <XCircle onClick={() => setShowAdmin(false)} cursor="pointer" size={22} color="#9ca3af" />
          </div>
          {!isPaid ? (
            <div style={styles.stepBox}>
              <input type="text" placeholder="Titre" style={styles.input} value={newTitre} onChange={e => setNewTitre(e.target.value)} />
              <input type="text" placeholder="Auteur" style={styles.input} value={newAuteur} onChange={e => setNewAuteur(e.target.value)} />
              ...
              <button onClick={handleActionPaiement} disabled={uploading} style={styles.btnWave}>Continuer</button>
            </div>
          ) : (
            <form onSubmit={finaliserPublication} style={styles.form}>
              ...
              <button type="submit" disabled={uploading} style={styles.btnSubmit}>Enregistrer</button>
            </form>
          )}
        </div>
      )}
      ...
      <div style={styles.list}>
        ...
        {/* On affiche les résultats seulement si search n'est pas vide */}
        ...
        {/* Message si on tape quelque chose qui n'existe pas */}
        ...
        {/* Message de bienvenue quand c'est vide */}
        {search.trim() === "" && (
          <div style={styles.emptyState}>
            <FileText size={40} color="#e5e7eb" />
            <p>Entrez une lettre pour rechercher</p>
          </div>
        )}
      </div>
...
const styles = {
  ...
  adminPanel: { backgroundColor: '#fff', padding: '20px', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.08)', marginBottom: '20px' },
  ...
  input: { padding: '14px', borderRadius: '12px', border: '1px solid #e5e7eb', fontSize: '16px' },
  inputAdmin: { padding: '10px', borderRadius: '10px', border: '1px solid #f3f4f6', fontSize: '13px' },
  btnWave: { backgroundColor: '#1d4ed8', color: 'white', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '700' },
  form: { display: 'flex', flexDirection: 'column', gap: '15px' },
  btnSubmit: { backgroundColor: '#059669', color: 'white', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '700' },
  ...
  emptyState: { textAlign: 'center', marginTop: '50px', color: '#9ca3af', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' },
  fab: { position: 'fixed', bottom: '25px', left: '50%', transform: 'translateX(-50%)', backgroundColor: '#111827', color: 'white', border: 'none', padding: '15px 25px', borderRadius: '35px', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: '700', boxShadow: '0 10px 20px rgba(0,0,0,0.2)', zIndex: 100 }
};
